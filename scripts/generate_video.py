...
SAJANG = ("흰색 LA 로고가 수놓인 네이비 야구모자를 쓰고 데님 멜빵바지에 크림색 스웨터를 입은 실사풍 "
          "갈색 줄무늬 고양이 붕어빵 노점 사장님(모든 장면에서 동일한 고양이)")
GOKAEK = ("목에 분홍 리본을 매고 베이지색 목도리를 두른 귀여운 실사풍 하얀 아기 고양이 손님"
          "(모든 장면에서 동일한 고양이, 사람은 등장하지 않음)")
...
SCENES = [
    ...
    f"{BG}. 클로즈업: {GOKAEK}이 앞발로 입가를 가리고 몸을 들썩이며 즐겁게 웃음을 터뜨리는 장면, "
    f"뒤로 노점 보케",

    f"{BG}. 미디엄샷: {SAJANG}의 앞발이 물고기 그림이 그려진 따뜻한 종이봉투를 "
    f"{GOKAEK}의 내민 앞발에 건네주는 장면",
