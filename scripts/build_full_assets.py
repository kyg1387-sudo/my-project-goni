...
DOYUN = "20대 후반 한국 남성 배달 기사(짧은 검은 머리, 글자나 로고가 전혀 없는 무지 파란색 배달 점퍼, 검은 바지, 짜장 소스 묻은 흰 운동화)"
# 클로즈업용: 소품 묘사(운동화 등)를 빼서 모델이 소품을 손에 들고 나오는 오생성을 막는다
DOYUN_FACE = "20대 후반 한국 남성 배달 기사(앳된 둥근 얼굴, 자연스러운 검은 앞머리, 짧은 검은 머리, 글자나 로고가 전혀 없는 무지 파란색 배달 점퍼)"
MIRAN = "50대 한국 여성(둥근 얼굴, 짧고 단정한 검은 올림머리, 베이지색 트위드 재킷 정장과 흰 블라우스, 진주 목걸이)"
...
STYLE = ("시네마틱 한국 드라마, 실사 영화 화질, 동일한 인물과 의상과 장소를 "
         ...
         "말하는 입 모양은 한국어(영어 없음), 화면 속 옷·간판·소품·배경에는 글자나 로고가 보이지 않게")

# 스타일: D(도윤)/V(빌런)/J(정실장)
#   N  = 보이스오버 — 사용 금지(시청자 혼란). 인용문도 C(자막 카드)로 표시할 것
...
SECTIONS = [
    dict(
        ...
        lines=[
            (A, "5", f"{LOBBY}, {MIRAN}이 서서 짜장면 그릇을 기울여 검은 소스를 바닥에 쏟고, 그 앞에 {DOYUN}가 "
                     f"무릎을 꿇고 고개를 숙인 모습, 뒤에서 {TAESEOK}이 휴대폰으로 촬영하며 비웃는 표정, "
                     f"화면에는 정확히 세 명만 등장, {MUTE}", None),
    ...
    dict(
        ...
        speaker_shots={},
        lines=[
            (C, "", "『밑바닥에서 90일을 버텨라. 하루라도 정체를 밝히면,\\N그룹 전체를 재단에 기부한다.』 — 강 회장의 유언", None),
    ...
    dict(
        ...
        narration_shots=[
            ...
            f"새벽 산 중턱, 잔디로 덮인 둥근 봉분 앞에 화강암 비석이 하나만 서 있고, 비석 정면에는 큰 한자 '墓' 한 글자만 또렷하게 새겨져 있으며 그 외 다른 글자나 무늬는 전혀 없음, 그 앞에서 배달 기사 복장의 {DOYUN}가 짜장면 그릇을 내려놓고 고개 숙이는 장면, 일출, 감성적인 엔딩, {MUTE}",
        ],
        speaker_shots={
        ...
        lines=[
            (C, "", "한 시간 뒤", None),
            (A, "10", f"{LOBBY}, 바닥에 무릎을 꿇고 고개 숙인 사람은 {MIRAN}과 {TAESEOK} 두 명이고, "
                      f"{DOYUN}는 무릎 꿇지 않고 그 옆에 똑바로 서서 지켜보며, 정장 차림의 중년 남성 임원이 "
                      f"걸어 나오다 무릎 꿇은 두 사람을 보고 멈춰 서는 장면, 배달 기사는 반드시 서 있는 자세, {MUTE}", None),
            (D, "", "정 실장님. 오토바이는 제가 몰고 갈게요. 마지막 배달 하나 남았어요.", None),
            (J, "정 실장", "어디로 가십니까?", None),
            (D, "", "할아버지 산소요. 짜장면 좋아하셨거든요.", None),
            (C, "", "『밑바닥에서 무릎 꿇어본 사람만이,\\N남을 무릎 꿇리지 않는다.』 — 강 회장의 유언", None),
            (C, "", "— 다음 편에 계속 —", None),
        ],
    ),
]
