...
SECTIONS = [
    ...
    dict(
        ...
        shots=[
            # 얼굴 클로즈업에는 신발 묘사를 넣지 않는다 (모델이 신발을 화면에 합성하는 오해석 방지)
            "20대 후반 한국 남성 배달 기사(짧은 검은 머리, 파란색 배달 점퍼)의 얼굴 클로즈업, "
            "무표정하지만 깊은 눈빛, 어깨 위로만 보이는 정적인 구도, 화면 정지 같은 연출",
            "고급 한옥 서재에서 백발 노인의 손이 만년필로 유언장에 서명하는 회상 장면, 세피아 톤",
            "야간 도시 스카이라인과 대기업 본사 고층 빌딩 외관, 건물 상단 전광판에 "
            "한글 간판 \"한성그룹\" 네 글자가 크고 선명하게 빛나는 장면, 웅장한 야경, 간판 글자는 정확하고 또렷하게",
