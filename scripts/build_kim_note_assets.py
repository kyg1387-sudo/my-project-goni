...
SECTIONS = [
    ...
    dict(
        ...
        lines=[
            ...
            (A, "5", f"{CORR_DAWN}, {KIM}가 새 흰 무지 비닐봉지 하나를 문고리에 조심스럽게 걸고 "
                     f"손끝으로 한 번 매만지는 클로즈업(봉지는 화면에 이 하나뿐, 한 가지 동작만), "
                     f"{MUTE}", [REF_KIM]),
    ...
    dict(
        ...
        speaker_shots={
            K: [(f"{CORR_DAWN}, 문고리에 걸린 흰 무지 비닐봉지(안에 라벨 없는 무지 흰 반찬통들이 "
                 f"비쳐 보임)를 발견한 {KIM}가 옅게 웃다 한 손으로 허리를 짚는 모습, 어떤 용기에도 "
                 f"라벨·인쇄·글자 없음, 바닥에 눈 없음, 온화한 표정으로 {TALK}", [REF_KIM])],
        },
        lines=[
            ...
            (A, "5", f"문이 10센티미터만 살짝 열린 좁은 세로 틈 사이로 보이는 시점 샷, 틈의 양쪽은 "
                     f"어두운 문과 문틀이 크게 가리고, {CORR_DAWN} 저편 복도 끝을 향해 걸어가며 멀어지는 "
                     f"{KIM}의 완전한 뒷모습(등과 뒤통수만 보이고 얼굴은 전혀 보이지 않음, 카메라를 절대 "
                     f"돌아보지 않음), 바닥은 마른 회색 콘크리트 복도(눈 없음), {MUTE}", [REF_KIM_BACK]),
    ...
    dict(
        ...
        lines=[
            ...
            (A, "5", f"{GUARD_IN}, 정모 없이 맨머리로 간이침대에 눈을 감고 반듯이 누워 앓는 {KIM}, 창백하고 "
                     f"지친 얼굴, 머리와 이마에는 아무것도 쓰거나 얹지 않았고 정모는 옆 책상 위에 놓여 있음, "
                     f"제복 상의를 입은 채, 혼자 단독 인물 샷, {MUTE}", [REF_KIM]),
