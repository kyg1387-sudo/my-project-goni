    ...
    dict(
        ...
        speaker_shots={
            ...
            K: [(f"{GUARD_IN}, 정모를 벗은 맨머리에 물수건을 한 손에 쥔 채 간이침대에서 몸을 일으킨 "
                 f"{KIM}가 믿기지 않는 놀란 표정으로 {TALK}", [REF_KIM])],
        },
        lines=[
    ...
    dict(
        ...
        speaker_shots={
            JH: [(f"{APT} 경비실 앞, 기준 사진과 완전히 동일한 얼굴의 {JUNHO_SUIT}가 처음으로 밝고 "
                  f"또렷한 표정으로 {TALK}, 옷에 명찰·배지·핀 없음, 바닥은 눈이 전혀 없는 마른 보도블록",
                  [REF_JUNHO_SUIT])],
        ...
        lines=[
            ...
            (A, "10", f"{APT} 경비실 앞, {JUNHO_SUIT}가 {KIM}에게 허리 숙여 꾸벅 인사하고 {KIM}가 "
                      f"그의 어깨를 가볍게 툭 두드리는 장면, 바닥은 눈이 전혀 없는 마른 보도블록, "
            ...
            (A, "5", f"{APT} 길, {JIWOO}가 환하게 손을 흔들고 {PARK}이 반찬통 몇 개를 안고 걸어오는 "
                     f"장면, 바닥은 눈이 전혀 없는 마른 봄 보도, 화면에는 정확히 두 명만 등장, {MUTE}",
             [REF_JIWOO, REF_PARK]),
